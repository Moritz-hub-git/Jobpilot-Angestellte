...
### D14 · Hosting- und Datenhaltungs-Linie (Antwort auf Moritz' Vercel-/DB-Frage, 27.07.)
**Keine „richtige" Datenbank, kein Vercel — bewusst.** Datenbestand ist `jobs/jobs.yaml` + strukturierte Dateien im privaten Git-Repo („Git-as-Database"): versioniert, diff-bar, mein einziges sessionübergreifendes Gedächtnis, null Betriebsaufwand, keine Daten bei weiteren Dritten (Regel 4). Das Cockpit ist ein privates Claude-Artifact (nur Moritz' Account) und wird täglich neu generiert — Hosting-Problem existiert nicht. Schwellen für Wiedervorlage: (a) > ~200 aktiv gepflegte Stellen oder spürbar lahme Auswertungen → **SQLite im Repo** (echte Queries, weiterhin nur eine Datei, kein Dritter); (b) Moritz will ausdrücklich eine eigenständige Web-App mit Login → Vercel + gehostete DB nur nach seiner expliziten Freigabe (Kosten! Eskalationspflicht) und nur mit Zugriffsschutz; Quelle der Wahrheit bliebe das Repo, die App wäre generierte Ansicht. Empfehlung dokumentiert: aktuell klar dagegen.
